  ...
  if (!empty($_POST)) {
      ...
      // if ($dataTotal) {
          ...
          try {
              ...
              $stmt = $dbh->prepare("SELECT idEmpleado, numEmpleado, nombre, idDepartamento, idPuesto, correo, celular, activo, asignableAct
                                    FROM empleado
         ...
         try {
          ...
          try {
              // Funcion para llenar Selector de puesto de formulario
              $id = cleanInput($_POST['idProyecto']);
              $stmt = $dbh-> prepare("SELECT idCliente, proyecto.nombre, proyecto.descripcion, proyecto.idTipoProyecto , cobrarA, ventasPotenciales, PO, qtoNumber, idCuenta,
                                              tracking, idRespDiseno, idRespManu, sobreCarga, fechaReqCliente, fechaPromesa, fechaTentativa,
          ...
          try {
            ...
            try {
                ...
                $stmt = $dbh->prepare("UPDATE actividades_proyecto SET ubicacion = '$ubicacion', notas = '$notas'
                                        WHERE idActividades_proyecto = $id");
                $stmt->execute();
                $stmt2 = $dbh-> prepare("SELECT idActividades_proyecto, ubicacion, notas
                                         FROM actividades_proyecto
                                         WHERE idActividades_proyecto = $id");
                $stmt2->execute();
                $data['result'] = $stmt2->fetch();
