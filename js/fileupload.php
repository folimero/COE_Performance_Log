...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(isset($_FILES["anyfile"]) && $_FILES["anyfile"]["error"] == 0){
        $allowed = array("pdf", "xls", "xlsx", "xlsm", "csv", "msg", "rar");
        // $allowed = array("jpg" => "image/jpg", "jpeg" => "image/jpeg", "gif" => "image/gif", "png" => "image/png");
        $filename = str_replace(" ", "_", $_FILES["anyfile"]["name"]);
            ...
            if(file_exists($_SERVER['DOCUMENT_ROOT'] . "/images/quotes/" . $idCotizacion . "/" . $filename)){
                ...
                if(move_uploaded_file($_FILES["anyfile"]["tmp_name"], $_SERVER['DOCUMENT_ROOT'] . $dir . "/" . $filename)){
                    ...
                    try {
                        ...
                        if ($stmt->execute()) {
                            $stmtResult = $dbh-> prepare("SELECT ca.idCotizacionArchivo, ca.nombre AS aNombre, ca.tamano, cat.tipo, e.nombre AS eNombre, ca.fechaCrea
