<?php
  ...
  $nombreComplejidad=cleanInput($_POST['nombreComplejidad']);
  ...
  if ($dbh!=null) {  //Se logró la conexión con la BD
      ...
      $stmt2 = $dbh-> prepare("SELECT nombre FROM complejidad WHERE nombre=:nombreComplejidad");
      $stmt2->bindParam(':nombreComplejidad', $nombreComplejidad);
      $stmt2->execute();
      $result = $stmt2->fetchAll();
      // En caso de ya existir se muestra un error al usuario
      if ($result!=null) {
          $message = "REGISTRO NO COMPLETADO!. La complejidad " . $nombreComplejidad . " ya existe en la base de datos.";
          echo "<script>
                  alert('$message');
      ...
      elseif (empty($nombreComplejidad || empty($horas))) {
      ...
      // En caso de que haya pasado todas las validaciones, se procede a insetar el registro en la base de datos
          ...
          $stmt = $dbh-> prepare("INSERT INTO complejidad (nombre, horas) VALUES (?, ?)");
          // Se asignan los valores a la consulta preparada
          $stmt->bindParam(1, $nombreComplejidad);
          $stmt->bindParam(2, $horas);
