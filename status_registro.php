<?php
  ...
  if ($dbh!=null) {  //Se logró la conexión con la BD
      ...
      $stmt2 = $dbh-> prepare("SELECT nombre FROM status WHERE nombre=:nombre");
      $stmt2->bindParam(':nombre', $nombre);
      $stmt2->execute();
      $result = $stmt2->fetchAll();
      // En caso de ya existir se muestra un error al usuario
      if ($result!=null) {
        $message = "REGISTRO NO COMPLETADO!. El Status " . $nombre . " ya existe en la base de datos.";
        echo "<script>
                    alert('$message');
      ...
      // En caso de que haya pasado todas las validaciones, se procede a insetar el registro en la base de datos
          ...
          $stmt = $dbh-> prepare("INSERT INTO status (nombre, descripcion) VALUES (?, ?)");
