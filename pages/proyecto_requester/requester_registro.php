<?php
  ...
  if ($dbh!=null) {  //Se logró la conexión con la BD
      ...
      $stmt2 = $dbh-> prepare("SELECT nombre FROM proyecto_requester WHERE nombre=:nombre");
      $stmt2->bindParam(':nombre', $nombre);
      $stmt2->execute();
      $result = $stmt2->fetchAll();
      // En caso de ya existir se muestra un error al usuario
      if ($result!=null) {
          $message = "RECORD NOT SAVED!. service " . $nombre . " already exists.";
          echo "<script>
                  alert('$message');
                  window.location.href='proyecto_requester.php';
              </script>";
      }
      ...
      // En caso de que haya pasado todas las validaciones, se procede a insetar el registro en la base de datos
          ...
          $stmt = $dbh-> prepare("INSERT INTO proyecto_requester (nombre) VALUES (?)");
