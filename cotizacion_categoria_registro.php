<?php
  ...
  if ($dbh!=null) {  //Se logró la conexión con la BD
      ...
      $stmt2 = $dbh-> prepare("SELECT categoria FROM cotizacion_categoria WHERE categoria=:nombreCategoria");
      ...
      if ($result!=null) {
          ...
          echo "<script>
                  alert('$message');
                  window.location.href='cotizacion_categoria.php';
              </script>";
          die();
      }
      // Valida que ningun campo este vacio
      elseif (empty($nombreCategoria) || empty($complejidad) || empty($descripcion)) {
      ...
      // En caso de que haya pasado todas las validaciones, se procede a insetar el registro en la base de datos
          ...
          $stmt = $dbh-> prepare("INSERT INTO cotizacion_categoria (categoria, descripcion, complejidad) VALUES (?, ?, ?)");
